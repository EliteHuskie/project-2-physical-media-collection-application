...
async function searchMoviesAndTVShows() {
    ...
    const movieUrl = `https://api.themoviedb.org/3/search/movie?api_key=${tmdbApiKey}&query=${encodeURIComponent(searchQuery)}`;
    const tvShowUrl = `https://api.themoviedb.org/3/search/tv?api_key=${tmdbApiKey}&query=${encodeURIComponent(searchQuery)}`;
...
async function searchBooks() {
    const searchQuery = document.getElementById('searchInput').value;
    const url = `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(searchQuery)}&key=${googleApiKey}`;
...
function displayResults(results, tvShows, type) {
    ...
    if (type === 'moviesAndTVShows') {
        results.forEach(item => {
            const resultCard = createResultCard(item.title, item.poster_path, item.overview, null, null, type);
            resultsContainer.appendChild(resultCard);
        });
        
        tvShows.forEach(item => {
            const resultCard = createResultCard(item.name, item.poster_path, item.overview, null, null, type);
            resultsContainer.appendChild(resultCard);
        });
    } else if (type === 'books') {
        results.forEach(book => {
            const resultCard = createResultCard(book.volumeInfo.title, null, book.volumeInfo.description, book.volumeInfo.authors, book.volumeInfo.imageLinks.thumbnail, type);
            resultsContainer.appendChild(resultCard);
        });
    }
}
...
function createResultCard(title, posterPath, overview, authors, thumbnail, type) {
